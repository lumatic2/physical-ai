"""CoM-aware QP-lite selector for the G1 visible squat gate."""
...
def write_summary(result: dict[str, Any], out_dir: Path) -> None:
    ...
    (out_dir / "com-qplite-summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    out_dir = VERIFY / "com-qplite-selector"
    out_dir.mkdir(parents=True, exist_ok=True)
    gains = {
        "pitch": 2.0,
        "ankle_pitch": -1.4,
    ...
    variants = [
        {"attempt": "balanced-0p08", "drop": 0.08, "max_blend": 0.70, "descend_s": 3.2, "return_s": 1.6, "mode": "balanced", "feedback_grid": [0.0, 0.5, 1.0]},
        {"attempt": "feedback-fixed-0p08", "drop": 0.08, "max_blend": 0.55, "descend_s": 3.0, "return_s": 1.6, "mode": "depth", "feedback_grid": [1.0]},
        {"attempt": "depth-0p08", "drop": 0.08, "max_blend": 0.75, "descend_s": 3.4, "return_s": 1.7, "mode": "depth", "feedback_grid": [0.0, 0.5, 1.0]},
        {"attempt": "strict-0p10", "drop": 0.10, "max_blend": 0.80, "descend_s": 3.6, "return_s": 1.8, "mode": "strict", "feedback_grid": [0.0, 0.5, 1.0]},
        {"attempt": "depth-0p10", "drop": 0.10, "max_blend": 0.85, "descend_s": 3.8, "return_s": 1.8, "mode": "depth", "feedback_grid": [0.0, 0.5, 1.0, 1.3]},
        {"attempt": "depth-0p12", "drop": 0.12, "max_blend": 0.90, "descend_s": 4.0, "return_s": 1.8, "mode": "depth", "feedback_grid": [0.0, 0.5, 1.0, 1.3]},
    ]
    result = {
        "planning_gate": {
            "team_validation_mode": "manual-pass",
            "spec_delta": "M19 probes a short-horizon selector over blend and CoM feedback scale, using height/support/ZMP/contact-force costs.",
            "perspectives": {
                "product": "directly pursues the native visible squat gate",
                "architecture": "reuses exp44 one-step selector and exp55 CoM feedback instead of adding another PPO loop",
                ...
                "skeptic": "one-step lookahead may still choose shallow controls to avoid delayed falls",
            },
            "dod": [
