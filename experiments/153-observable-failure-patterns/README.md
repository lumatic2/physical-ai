...
Step 5는 27/27 timeout을 policy·suite별로 다시 집계했다. `no_progress`는 6개(22.2%), `unknown`은 21개(77.8%)다. 분모 누락·unknown 은폐·root-cause 진단·독립 인간·실물 로봇 과장 claim은 gate에서 거부된다.
...
python experiments/153-observable-failure-patterns/coverage_gate.py
python experiments/153-observable-failure-patterns/test_coverage_gate.py
```
