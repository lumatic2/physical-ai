"""Probe safe-basis residual filters on top of the exp55 CoM-feedback teacher."""
...
def write_summary(result: dict[str, Any], out_dir: Path) -> None:
    ...
    (out_dir / "safe-basis-residual-summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    out_dir = VERIFY / "safe-basis-residual"
    out_dir.mkdir(parents=True, exist_ok=True)
    base_gains = {
        "pitch": 2.0,
        "ankle_pitch": -1.4,
    ...
    variants = [
        {"attempt": "teacher-best", "source": "com", "signs": signs_a, "max_blend": 0.50, "adapt_gain": 0.14, "descend_s": 3.0, "pattern": "none", "scale": 0.0, "filter": "none"},
        {"attempt": "unfiltered-safe-combo-r0p06", "source": "com", "signs": signs_a, "max_blend": 0.50, "adapt_gain": 0.14, "descend_s": 3.0, "pattern": "safe_combo", "scale": 0.06, "filter": "none"},
        {"attempt": "soft-safe-combo-r0p06", "source": "com", "signs": signs_a, "max_blend": 0.50, "adapt_gain": 0.14, "descend_s": 3.0, "pattern": "safe_combo", "scale": 0.06, "filter": "soft"},
        {"attempt": "zmp-hold-safe-combo-r0p08", "source": "com", "signs": signs_a, "max_blend": 0.50, "adapt_gain": 0.14, "descend_s": 3.0, "pattern": "safe_combo", "scale": 0.08, "filter": "zmp_hold"},
        {"attempt": "zmp-hold-counter-ankle-r0p08", "source": "com", "signs": signs_a, "max_blend": 0.50, "adapt_gain": 0.14, "descend_s": 3.0, "pattern": "counter_ankle", "scale": 0.08, "filter": "zmp_hold"},
        {"attempt": "soft-ankle-recenter-r0p08", "source": "com", "signs": signs_a, "max_blend": 0.50, "adapt_gain": 0.14, "descend_s": 3.0, "pattern": "ankle_recenter", "scale": 0.08, "filter": "soft"},
    ]
    result = {
        "planning_gate": {
            "team_validation_mode": "manual-pass",
            "spec_delta": "M19 probes a support/ZMP/slip-gated safe-basis residual filter before launching a longer residual fine-tune.",
            "perspectives": {
                "product": "tests the next visible squat blocker identified in ROADMAP",
                "architecture": "wraps exp55 teacher and minimally filters residual basis actions using support, ZMP, and slip health",
                "security": "no credentials or external side effects",
                "qa": "native sweep logs CoM/ZMP margins, stance, pose, contact, return, fall",
                "skeptic": "a simple heuristic filter may preserve safety only by suppressing all useful depth residual",
            },
            "dod": [
                "native JSON per CoM/ZMP feedback variant",
                "summary identifies whether safe residual filtering deepens the exp55 teacher without breaking M19 gates",
            ],
        },
        "runs": [],
    }
