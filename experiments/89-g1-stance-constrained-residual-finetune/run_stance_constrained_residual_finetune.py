"""Finetune exp80 visible geometry with stance-constrained residual rewards."""
...
from g1_stance_constrained_residual_env import StanceConstrainedResidualSquat  # noqa: E402
...
def make_env(target_drop: float, support_floor: float, slip_limit: float) -> StanceConstrainedResidualSquat:
    return StanceConstrainedResidualSquat(
...
def write_summary(result: dict, out_dir: Path) -> None:
    ...
    lines = [
        "# G1 Stance-Constrained Residual Finetune Summary",
        "",
        ...
        "",
        "This experiment starts from exp80 visible geometry and asks whether stance-constrained residual rewards can recover contact/slip/terminal stand without erasing the visible gate.",
    ]
    (out_dir / "stance-constrained-residual-summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    result = {
        "planning_gate": {
            ...
            "spec_delta": "M19 now tests stance-constrained residual finetuning from the exp80 visible-geometry checkpoint.",
            "perspectives": {
                "product": "targets the current M19 blocker: visible geometry exists, but contact/slip/return are not deployable",
                "architecture": "reuses exp80 evaluator and checkpoint shape; env adds stance residual budget and hard contact/slip health rewards",
                "security": "no secrets or external credentials",
                "qa": "compatibility, rollout smoke, restored PPO, native exp29 visible gate audit",
                "skeptic": "hard stance residual rewards may erase visible depth or still fail to anchor feet in native MuJoCo",
            },
            "dod": [
                "stance residual reward metrics execute",
        ...
        "web_sources": [
            {
                "url": "https://arxiv.org/html/2510.05070v1",
                "accessed": "2026-06-18",
                "use": "Residual humanoid learning motivates refining a broad tracking policy with task-specific residual constraints.",
            },
            {
                "url": "https://arxiv.org/html/2501.02116v1",
                "accessed": "2026-06-18",
                "use": "Humanoid whole-body manipulation survey notes pure learning lacks flexibility around contacts, motivating integrated constraints.",
            },
            {
