"""Train and evaluate stance-constrained command-conditioned G1 squat policies."""
...
from g1_stance_constrained_env import StanceConstrainedCommandSquat
...
def default_source() -> Path:
    return EXP49_P03 if EXP49_P03.exists() else EXP46_PARAMS
...
def make_env(target_drop: float, support_floor: float, slip_limit: float) -> StanceConstrainedCommandSquat:
    return StanceConstrainedCommandSquat(
...
def rollout_smoke(target_drop: float, support_floor: float, slip_limit: float, steps: int = 20) -> dict:
    ...
    for _ in range(steps):
        ...
        slips.append(float(state.metrics["stance_foot_slip"]))
        supports.append(float(state.metrics["stance_support_margin"]))
        stance_terms.append(float(state.metrics["stance_terminated"]))
        dones.append(float(state.done))
    return {
        "rollout_steps": steps,
        "reward_first": rewards[0],
        "reward_last": rewards[-1],
        "foot_slip_max": max(slips),
        "support_margin_min": min(supports),
        "stance_terminated_any": any(v > 0 for v in stance_terms),
...
def build_policy(env: StanceConstrainedCommandSquat, params_path: Path):
...
def native_eval(target_drop: float, support_floor: float, slip_limit: float, params_path: Path, seconds: float, out_dir: Path) -> dict:
    ...
    if pass_gate:
        verdict = "PASS_STANCE_CONSTRAINED_VISIBLE_SQUAT"
...
def write_summary(result: dict, out_dir: Path) -> None:
    ...
    lines = [
        "# G1 Stance-Constrained Curriculum PPO Summary",
        "",
        "| Verdict | Timesteps | Train min | Drop | Fell at | Final h | Contact | Slip | Support min | First support | First slip |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
        (
            ...
            f"{native['first_support_breach_at']} | {native['first_slip_breach_at']} |"
        ),
        "",
        "M19 closes only when visible depth, no-fall, contact, stance, return, knee/hip pose, and browser replay gates pass together.",
    ]
    (out_dir / "stance-constrained-summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--source", type=Path, default=None)
    parser.add_argument("--target-drop", type=float, default=0.03)
    ...
    out_dir = VERIFY / f"target-{args.target_drop:.2f}-slip-{args.slip_limit:.2f}".replace(".", "p")
    out_dir.mkdir(parents=True, exist_ok=True)
    result = {
        "planning_gate": {
            "team_validation_mode": "manual-pass",
            "spec_delta": "M19 PPO loop now treats support/slip breach as episode-level failure and adds stance-lock action prior.",
            "perspectives": {
                "product": "targets the exact exp49 failure mode: large slip and contact loss",
                "architecture": "small subclass of exp49 env; restored checkpoint shape preserved",
                "security": "no secrets or external credentials",
                "qa": "compatibility, rollout smoke, restored PPO, native visible gate audit",
                "skeptic": "termination may make learning overly conservative and return to shallow standing",
            },
            "dod": [
                "support/slip termination metrics execute",
