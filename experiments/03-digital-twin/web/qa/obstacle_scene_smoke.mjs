// Verifies that the obstacle scenario uses an active MJCF scene with obstacle geoms.
//
// Usage:
//   node qa/obstacle_scene_smoke.mjs --exp=g1-obstacle-walk --scenario=obstacle-lane-v1
//   node qa/obstacle_scene_smoke.mjs --live --exp=g1-obstacle-walk --scenario=obstacle-lane-v1
...
const EVIDENCE_DIR = join(REPO_ROOT, 'experiments', '142-interactive-obstacle-scene', 'verify');
const OUT_DIR = join(QA_DIR, 'out');
const args = process.argv.slice(2);
const live = args.includes('--live');
const exp = (args.find((a) => a.startsWith('--exp=')) || '--exp=g1-obstacle-walk').split('=')[1];
const scenario = (args.find((a) => a.startsWith('--scenario=')) || '--scenario=obstacle-lane-v1').split('=')[1];
const PORT = 8141;
...
async function main() {
  ...
  evidence.pass = Boolean(
    ...
    summary.scenario?.obstacle?.enabled === true &&
    summary.scenario?.obstacle?.sceneGeomCount >= 3 &&
    summary.scenario?.terrain?.kind === 'offset-obstacle-lane' &&
    /obstacle/i.test(summary.scene?.activeScene || '') &&
    statusText.includes(scenario) &&
    statusText.includes('offset-box-lane') &&
    consoleErrors.length === 0
  );

  const suffix = live ? 'live' : 'local';
  const outPath = join(EVIDENCE_DIR, `obstacle-scene-smoke-${suffix}.json`);
  writeFileSync(outPath, JSON.stringify(evidence, null, 2));
  await page.screenshot({ path: join(OUT_DIR, `obstacle_scene_${suffix}.png`) });
  await browser.close();
