// Verifies the debug-only Physics diagnostics panel against runtime readout data.
//
// Usage:
...
const evidencePath = outArg
  ? outArg.split('=')[1]
  : join(WEB_DIR, '..', '..', '137-physics-diagnostics-panel', 'verify', 'physics-diagnostics-panel-smoke.json');
const PORT = 8132;
...
function readEvidence() {
  if (!existsSync(evidencePath)) {
    return {
      ...
      claimBoundary: 'visible read-only MuJoCo WASM runtime diagnostics, not real robot telemetry',
      local: null,
      live: null,
    };
  }
...
async function main() {
  ...
  await page.waitForFunction(() => window.demo.qaWorkbenchSummary().physicsReadout?.enabled === true, null, {
    timeout: 30000,
    polling: 250,
  });
  ...
  result.pass = Boolean(
    ...
    uiVisible &&
    /physics diagnostics/i.test(uiText) &&
    /contact count/i.test(uiText) &&
    /supported fields/i.test(uiText) &&
    /cfrc_ext sample/i.test(uiText) &&
    /sensordata sample/i.test(uiText) &&
    /read-only MuJoCo WASM runtime probe/i.test(uiText) &&
    /not real robot telemetry/i.test(uiText) &&
    consoleErrors.length === 0
  );

  await page.screenshot({ path: join(OUT_DIR, `${exp}_${live ? 'live' : 'local'}_physics_diagnostics_panel.png`) });
  await browser.close();

  const evidence = readEvidence();
  evidence.milestone = 'M36';
  evidence.experiment = exp;
  evidence.requestedPreset = preset;
  evidence.claimBoundary = 'visible read-only MuJoCo WASM runtime diagnostics, not real robot telemetry';
