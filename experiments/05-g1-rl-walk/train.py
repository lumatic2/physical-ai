"""G1JoystickFlatTerrain 정책 학습 (brax PPO, Playground 튜닝 config).

experiments/04-go1-rl-walk/train.py 의 G1 버전 — ENV만 다르고 파이프라인 동일.
locomotion_params 를 그대로 미러링하되 mujoco_warp(미설치) 회피로 impl="jax"(클래식 MJX) 오버라이드.
...
OUT = sys.argv[1] if len(sys.argv) > 1 else "/home/yusun/playground-go1/runs/g1flat"
