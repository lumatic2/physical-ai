"""Fine-tune squat depth from the exp21 stabilizer checkpoint."""
...
from g1_squat_depth_env import G1SquatDepth, REFERENCE


EXP_DIR = Path(__file__).resolve().parent
ROOT = EXP_DIR.parents[1]
VERIFY = EXP_DIR / "verify"
DEFAULT_SOURCE = ROOT / "experiments/21-g1-stabilizer-init-probe/verify/train/params.pkl"
...
def compatibility(source: Path) -> dict:
    env = G1SquatDepth(config_overrides={"impl": "jax"})
    cfg = ppo_config(1)
...
def rollout_smoke(steps: int = 20) -> dict:
    ...
    for _ in range(steps):
        ...
        height_errors.append(float(state.metrics["height_error"]))
    return {
        ...
        "height_error_last": height_errors[-1],
        "done_any": any(v > 0 for v in dones),
    }


def train(source: Path, timesteps: int) -> dict:
    env = G1SquatDepth(config_overrides={"impl": "jax"})
    eval_env = G1SquatDepth(config_overrides={"impl": "jax"})
    ...
    (out_dir / "rewards.txt").write_text(
        "\n".join([f"# timesteps={timesteps} train_min={elapsed/60:.2f} source={source}"] + [f"{s}\t{r}" for s, r in rewards]) + "\n",
        encoding="utf-8",
    )
    return {
        "timesteps": timesteps,
...
def build_policy(env: G1SquatDepth, params_path: Path):
...
def native_eval(seconds: float) -> dict:
    ...
    depth_pass = fell_at is None and min_height < 0.70
    return {
        "baseline_exp21_min_height": 0.7522832074616519,
        "target_depth_height": 0.70,
        "engine": "native mujoco.MjData closed-loop depth fine-tune from exp21 stabilizer",
        "seconds": seconds,
        "fell_at": fell_at,
        "upright_s": upright_s,
        "min_height": min_height,
        "final_height": float(data.qpos[2]),
        "improved_depth_vs_exp21": min_height < 0.7522832074616519,
        ...
        "verdict": "PASS_DEPTH" if depth_pass else ("IMPROVED_DEPTH_PENDING" if fell_at is None and min_height < 0.7522832074616519 else "FAIL_DEPTH"),
        "samples": samples,
    }
