"""Depth-focused G1 squat env initialized from a stabilizer policy."""
...
class G1SquatDepth(joystick.Joystick):
    """Reference tracking with stronger base-height/depth pressure."""
    ...
    def _get_reward(
        ...
        desired_drop = jp.maximum(self.stand_height - ref_height, 1e-3)
        actual_drop = jp.clip(self.stand_height - data.qpos[2], 0.0, desired_drop)
        depth_progress = actual_drop / desired_drop
        ...
        return {
            "alive": jp.ones(()),
            "reference_tracking": jp.exp(-8.0 * joint_error),
            "arm_still": jp.exp(-4.0 * arm_error),
            "height_tracking": jp.exp(-120.0 * height_error),
