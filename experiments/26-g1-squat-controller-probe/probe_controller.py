"""Probe stabilizer-preserving controller blends for G1 squat stage 0.74.

This experiment does not train a new policy. It asks whether the already stable
G1 policy can be blended with a shallow staged squat reference without falling.
"""
...
def variant_ctrl(
    ...
    if name == "policy_only":
        return clip_ctrl(model, policy_ctrl)
    if not name.startswith("blend_"):
        raise ValueError(f"unknown variant {name}")
    blend = float(name.removeprefix("blend_").replace("p", "."))
...
def write_report(result: dict) -> None:
    ...
    (VERIFY / "g1-squat-controller-probe.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> None:
    ...
    parser.add_argument(
        "--variants",
        nargs="+",
        default=["policy_only", "blend_0p10", "blend_0p20", "blend_0p35", "blend_0p50"],
    )
    ...
    result = {
        "planning_gate": {
            ...
            "spec_delta": "exp25 depth curriculum refined into controller arbitration probe",
            "dod": [
                "policy-only and blend variants evaluated with identical native metrics",
        ...
        "next": "Use a stable depth blend as curriculum controller; if none reaches depth, redesign action arbitration or run longer fine-tune with behavior cloning toward staged pose.",
    }
    (VERIFY / "g1-squat-controller-probe.json").write_text(json.dumps(result, indent=2), encoding="utf-8")
    write_report(result)
