...
class ContactAwareSquat(EXP25.G1SquatCurriculum):
    ...
    def __init__(self, *args, controller_blend: float = 0.0, freeze_phase: bool = False, **kwargs):
    ...
    def _motor_targets(self, action: jax.Array, step: jax.Array) -> jax.Array:
        ...
        return (1.0 - self._controller_blend) * policy_targets + self._controller_blend * staged_pose
...
def compatibility(source: Path, stage_height: float, controller_blend: float, freeze_phase: bool) -> dict:
...
def rollout_smoke(stage_height: float, controller_blend: float, freeze_phase: bool, steps: int = 20) -> dict:
    env = ContactAwareSquat(
        stage_height=stage_height,
        controller_blend=controller_blend,
        freeze_phase=freeze_phase,
...
def native_eval(
    ...
    for step in range(total_steps):
        ...
        data.ctrl[:] = (1.0 - controller_blend) * policy_targets + controller_blend * staged_pose
...
def main() -> None:
    ...
    result = {
        ...
        "source_params": str(source),
        "compatibility": compatibility(source, args.stage_height, args.controller_blend, args.freeze_phase),
        "rollout": rollout_smoke(args.stage_height, args.controller_blend, args.freeze_phase),
    }
