...
def native_eval(stage_height: float, params_path: Path, seconds: float, out_dir: Path) -> dict:
    env = ContactAwareSquat(stage_height=stage_height, config_overrides={"impl": "jax"})
...
def train(source: Path, stage_height: float, timesteps: int, attempt_dir: Path, seed: int) -> dict:
    env = ContactAwareSquat(stage_height=stage_height, config_overrides={"impl": "jax"})
    eval_env = ContactAwareSquat(stage_height=stage_height, config_overrides={"impl": "jax"})
...
def main() -> None:
    ...
    if args.train:
        result["train"] = train(source, args.stage_height, args.timesteps, attempt_dir, args.seed)
        params_path = attempt_dir / "train" / "params.pkl"
    else:
        params_path = source
    result["native"] = native_eval(args.stage_height, params_path, args.seconds, attempt_dir)
