"""Probe a phase-shaped knee-flexion micro-phase for the G1 visible squat gate."""
...
def knee_microphase_target(*args, **kwargs):
    ik_target = np.asarray(kwargs["ik_target"], dtype=np.float64).copy()
    if ACTIVE_MICRO:
        phase = float(kwargs["desired_fraction"])
        health = min(
...
EXP71.EXP62.build_target = knee_microphase_target
...
def variants() -> list[dict[str, Any]]:
    ...
    for knee_amp in [0.04, 0.06, 0.08, 0.10, 0.12]:
        for phase_start, phase_peak, phase_end in windows:
            for min_health in [0.45, 0.65]:
                hip_bias = 0.02
                rows.append({
                    **base,
                    "attempt": (
                        f"micro-k{knee_amp:.2f}-h{hip_bias:.2f}-"
    ...
    focused = [
        (0.07, 0.02, 0.35, 0.55, 0.75, 0.75),
        (0.07, 0.03, 0.35, 0.55, 0.75, 0.75),
        (0.08, 0.03, 0.35, 0.55, 0.75, 0.75),
        (0.09, 0.02, 0.45, 0.65, 0.85, 0.75),
        (0.09, 0.03, 0.45, 0.65, 0.85, 0.75),
        (0.10, 0.03, 0.55, 0.75, 0.95, 0.75),
        (0.10, 0.04, 0.55, 0.75, 0.95, 0.75),
        (0.11, 0.03, 0.55, 0.75, 0.95, 0.80),
    ]
    for knee_amp, hip_bias, phase_start, phase_peak, phase_end, min_health in focused:
        rows.append({
            **base,
            "attempt": (
                f"focus-k{knee_amp:.2f}-h{hip_bias:.2f}-"
                f"p{phase_start:.2f}-{phase_peak:.2f}-{phase_end:.2f}-"
                f"mh{min_health:.2f}"
            ).replace(".", "p"),
            "knee_amp": knee_amp,
    ...
    rows.sort(key=lambda row: (
        abs(row["knee_amp"] - 0.08),
        abs(row["phase_peak"] - 0.65),
        row["min_health"],
    ))
    return rows[:36]
...
def write_summary(result: dict[str, Any], out_dir: Path) -> None:
    ...
    (out_dir / "knee-flexion-microphase-summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    out_dir = VERIFY / "knee-flexion-microphase"
    out_dir.mkdir(parents=True, exist_ok=True)
    result: dict[str, Any] = {
        "evaluation_seconds": args.seconds,
        "planning_gate": {
            "team_validation_mode": "manual-pass",
            "spec_delta": "M19 tests a support-health-gated knee-flexion micro-phase in the IK target family.",
            "perspectives": {
                "product": "directly tests whether the Unitree G1 model can express the remaining knee motion while retaining squat recovery",
                "architecture": "patches EXP62.build_target and reuses the exp71 native evaluator/gates",
                "security": "local MuJoCo/JAX execution only",
                "qa": "native JSON per candidate plus visible 8cm, knee/hip, contact, slip, return gates",
                "skeptic": "desired_fraction is reused during return, so this probe may need a descend-only evaluator if results are ambiguous",
            },
            "dod": [
                "run phase-windowed knee/hip micro-target candidates around the exp73/75 best timing",
                "report whether achieved knee flexion exceeds the 0.513rad recoverable plateau without losing support/ZMP",
            ],
        },
        "sources": [
            {
                ...
                "accessed": "2026-06-18",
                "note": "Task-based WBC combines CoM/ZMP regulation, feet pose tasks, and joint objectives for humanoid posture transitions.",
            },
            {
                "url": "https://pmc.ncbi.nlm.nih.gov/articles/PMC11769464/",
                "accessed": "2026-06-18",
                "note": "Humanoid squatting is treated as whole-body coordination over a trajectory rather than an isolated joint target.",
            },
            {
                "url": "https://arxiv.org/html/2502.17219v1",
                "accessed": "2026-06-18",
                "note": "ZMP inside the support polygon is used as a balance condition for humanoid WBC.",
            },
            {
                "url": "https://www.roboticsproceedings.org/rss21/p070.pdf",
                "accessed": "2026-06-18",
                "note": "Recent Unitree G1 work includes squat-to-height behavior and motivates explicit knee-flexion shaping.",
            },
        ],
        "runs": [],
    }
