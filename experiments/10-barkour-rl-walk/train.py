"""Train BarkourJoystick with Brax PPO.

Usage:
  python train.py [out_dir] [num_timesteps]
"""
import functools
import os
import pickle
...
ENV = "BarkourJoystick"
OUT = sys.argv[1] if len(sys.argv) > 1 else "/home/yusun/playground-go1/runs/barkour"
...
print(
    ...
    "| net",
    tuple(ppo_params.network_factory.policy_hidden_layer_sizes),
    flush=True,
)
...
with open(os.path.join(OUT, "rewards.txt"), "w", encoding="utf-8") as f:
    f.write(
        f"# {ENV} impl=jax num_timesteps={ppo_params.num_timesteps} train_min={dt / 60:.1f}\n"
    )
