"""Predictive-sampling search around the G1 squat contact-safe trajectory branch."""
...
def optimizer_score(run: dict[str, Any]) -> float:
    gap = run["visible_gap"]
    score = 0.0
    score += 1300.0 if run["fell_at"] is not None else 0.0
    score += 420.0 * gap["drop_shortfall_m"] / 0.08
    score += 520.0 * gap["knee_shortfall_rad"] / 0.60
    score += 260.0 * gap["hip_shortfall_rad"] / 0.35
    score += 360.0 * gap["slip_excess_m"] / 0.08
    score += 260.0 * max(0.0, 0.90 - run["foot_contact_ratio"])
    score += 340.0 * max(0.0, 0.74 - run["final_height"])
    score += 120.0 * max(0.0, -run["min_support_margin"])
    score += 120.0 * max(0.0, -run["min_zmp_margin"])
...
def variants() -> list[dict[str, Any]]:
    ...
    teacher = {**common, "drop": 0.080, "joint_kp": 26.0, "torque_clip": 36.0, "foot_kp_xy": 500.0, "foot_force_clip": 380.0, "descend_s": 3.8, "recapture_s": 1.6, "return_s": 2.6, "horizon_steps": 10, "w_support": 9800.0, "w_slip": 8400.0, "w_stand": 880.0, "return_joint_kp": 46.0, "return_torque_clip": 66.0, "knee_amp": 0.08, "hip_bias": 0.14, "w_knee": 58.0, "w_hip": 320.0, "pose_health_floor": 0.16, "w_depth_cap": 22000.0, "recapture_drop": 0.10}
    seed = {**teacher, "attempt": "seed-sample15-visible-branch", "max_blend": 0.50046, "residual_scale": 0.04976, "w_height": 120.0, "depth_cap": 0.13303, "w_depth_cap": 87600.0, "w_contact": 1304.0, "w_slip": 32310.0, "w_stand": 1645.0, "w_knee": 294.0, "w_hip": 340.0, "return_joint_kp": 54.0, "return_torque_clip": 74.0, "foot_kp_xy": 570.0, "foot_force_clip": 505.5, "preload_force": 36.64, "preload_height_kp": 673.8, "preload_force_clip": 108.6, "knee_amp": 0.131, "hip_bias": 0.144, "pose_health_floor": 0.356, "descend_s": 4.305, "recapture_s": 2.085, "return_s": 3.240, "recapture_drop": 0.098, "horizon_steps": 8}
    fields = {
        "max_blend": (0.492, 0.508),
        "residual_scale": (0.044, 0.056),
        "depth_cap": (0.115, 0.150),
        "w_depth_cap": (76000.0, 115000.0),
        "w_contact": (1120.0, 1580.0),
        "w_slip": (29000.0, 44000.0),
        "w_stand": (1450.0, 2300.0),
        "w_knee": (300.0, 680.0),
        "w_hip": (330.0, 480.0),
        "foot_kp_xy": (560.0, 660.0),
        "foot_force_clip": (470.0, 540.0),
        "preload_force": (32.0, 42.0),
        "preload_height_kp": (640.0, 740.0),
        "preload_force_clip": (100.0, 120.0),
        "knee_amp": (0.125, 0.165),
        "hip_bias": (0.135, 0.160),
        "pose_health_floor": (0.34, 0.62),
        "descend_s": (4.1, 4.7),
        "recapture_s": (1.9, 2.4),
        "return_s": (3.1, 3.7),
        "recapture_drop": (0.080, 0.115),
        "pose_qfrc_scale": (0.0, 0.12),
        "pose_qfrc_kp": (0.0, 18.0),
        "pose_qfrc_clip": (0.0, 4.0),
        "pose_qfrc_health_floor": (0.62, 0.86),
    }
    rng = np.random.default_rng(9301)
    variants_out = [
        {**teacher, "attempt": "baseline-plan9-terminal", "max_blend": 0.500, "residual_scale": 0.052, "w_height": 120.0, "depth_cap": 0.120, "w_depth_cap": 90000.0, "w_contact": 1240.0, "w_slip": 27000.0, "w_stand": 1500.0, "w_knee": 300.0, "w_hip": 380.0, "return_joint_kp": 54.0, "return_torque_clip": 74.0, "foot_kp_xy": 610.0, "foot_force_clip": 470.0, "preload_force": 34.0, "preload_height_kp": 660.0, "preload_force_clip": 108.0, "knee_amp": 0.112, "hip_bias": 0.150, "pose_health_floor": 0.36, "descend_s": 4.3, "recapture_s": 2.1, "return_s": 3.3, "recapture_drop": 0.090},
        seed,
    ]
    for idx in range(1, 19):
        variant = {**seed, "attempt": f"sample-{idx:02d}"}
        for name, (lo, hi) in fields.items():
            center = float(seed.get(name, (lo + hi) / 2.0))
            scale = (hi - lo) / 4.0
            value = float(np.clip(rng.normal(center, scale), lo, hi))
            variant[name] = value
        if variant["pose_qfrc_scale"] < 0.035:
            variant["pose_qfrc_kp"] = 0.0
            variant["pose_qfrc_clip"] = 0.0
        variant["horizon_steps"] = int(rng.choice([8, 9, 10, 11, 12]))
...
def write_summary(result: dict[str, Any], out_dir: Path) -> None:
    ...
    (out_dir / "predictive-sampling-trajectory-search-summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    out_dir = VERIFY / "predictive-sampling-trajectory-search"
    ...
    result = {
        ...
        "planning_gate": {
            "team_validation_mode": "manual-pass",
                "spec_delta": "M19 replaces hand-built finite-horizon grids with seeded predictive sampling over trajectory parameters before attempting browser replay.",
            "perspectives": {
                "product": "tests whether derivative-free sampling can find a native visible squat plan that hand tuning missed",
                "architecture": "keeps exp87's 3-phase teacher and samples trajectory parameters around exp92's closest non-fall branch",
                "security": "no credentials or external side effects",
                "qa": "native sweep records raw JSON, visible gate metrics, sampled parameter candidates, and optimizer score per rollout",
                "skeptic": "single-batch predictive sampling may still be too small to cross the knee/contact/slip boundary",
            },
            "dod": [
                "raw native JSON per predictive-sampling candidate",
        ...
        "sources": [
            ...
            },
            {
                ...
                "accessed": "2026-06-18",
                "note": "Predictive Sampling is a simple derivative-free MJPC planner that motivates seeded rollout sampling for this local gate.",
            },
        ],
        "runs": [],
    }
