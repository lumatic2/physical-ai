"""Probe a safe-combo residual curriculum on top of the exp55 teacher."""
...
def apply_residual_pattern(
    ...
    *,
    ...
    adjusted = target.copy()
    if filter_mode == "none":
        knee_gate = float(np.clip(support_health, 0.0, 1.0))
        ankle_gate = knee_gate
    else:
        safety = float(np.clip(min(support_health, zmp_health, slip_health), 0.0, 1.0))
        knee_gate = safety * safety
        ankle_gate = max(knee_gate, 0.45 * float(np.clip(zmp_health, 0.0, 1.0)))
        if filter_mode == "zmp_hold" and np.linalg.norm(error_xy) > 0.045:
            knee_gate = 0.0
    amount = float(scale * phase_depth)
    if amount <= 0.0 or pattern == "none":
        return adjusted
    if pattern == "knee_only":
        deltas = {
            "left_knee_joint": knee_gate * amount,
            "right_knee_joint": knee_gate * amount,
        }
    elif pattern == "hip_knee":
        deltas = {
            "left_hip_pitch_joint": -0.45 * knee_gate * amount,
            "right_hip_pitch_joint": -0.45 * knee_gate * amount,
            "left_knee_joint": knee_gate * amount,
            "right_knee_joint": knee_gate * amount,
        }
    elif pattern == "counter_ankle":
        deltas = {
            "left_hip_pitch_joint": -0.35 * knee_gate * amount,
            "right_hip_pitch_joint": -0.35 * knee_gate * amount,
            "left_knee_joint": knee_gate * amount,
            "right_knee_joint": knee_gate * amount,
            "left_ankle_pitch_joint": -0.45 * ankle_gate * amount,
            "right_ankle_pitch_joint": -0.45 * ankle_gate * amount,
        }
    elif pattern == "safe_combo":
        deltas = {
            "left_hip_pitch_joint": -0.25 * knee_gate * amount,
            "right_hip_pitch_joint": -0.25 * knee_gate * amount,
            "left_knee_joint": 0.65 * knee_gate * amount,
            "right_knee_joint": 0.65 * knee_gate * amount,
            "left_ankle_pitch_joint": -0.80 * ankle_gate * amount,
            "right_ankle_pitch_joint": -0.80 * ankle_gate * amount,
        }
    elif pattern == "ankle_recenter":
        deltas = {
            "left_ankle_pitch_joint": -0.70 * ankle_gate * amount,
            "right_ankle_pitch_joint": -0.70 * ankle_gate * amount,
        }
    else:
...
def native_eval(
    *,
    ...
    for step in range(total_steps):
        ...
        support_health = float(np.clip((support["support_margin"] - support_floor) / 0.045, 0.0, 1.0))
        zmp_health = float(np.clip((zmp_margin - support_floor) / 0.045, 0.0, 1.0))
        slip_health = float(np.clip(1.0 - foot_slip / slip_limit, 0.0, 1.0))
        if return_phase > 0.0:
...
def write_summary(result: dict[str, Any], out_dir: Path) -> None:
    ...
    for run in result["runs"]:
        ...
        lines.append(
            f"| {run['attempt']} | {run['verdict']} | {run['feedback_source']}+{run['residual_pattern']} | {run['filter_mode']} | "
    ...
    (out_dir / "safe-combo-curriculum-summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    out_dir = VERIFY / "safe-combo-curriculum"
    out_dir.mkdir(parents=True, exist_ok=True)
    base_gains = {
        "pitch": 2.0,
        "ankle_pitch": -1.4,
    ...
    variants = [
        {"attempt": "teacher-0p08", "drop": 0.08, "source": "com", "signs": signs_a, "max_blend": 0.50, "adapt_gain": 0.14, "descend_s": 3.0, "pattern": "none", "scale": 0.0, "filter": "none"},
        {"attempt": "safe-combo-0p08-r0p06", "drop": 0.08, "source": "com", "signs": signs_a, "max_blend": 0.50, "adapt_gain": 0.14, "descend_s": 3.0, "pattern": "safe_combo", "scale": 0.06, "filter": "none"},
        {"attempt": "safe-combo-0p08-r0p09", "drop": 0.08, "source": "com", "signs": signs_a, "max_blend": 0.50, "adapt_gain": 0.14, "descend_s": 3.0, "pattern": "safe_combo", "scale": 0.09, "filter": "none"},
        {"attempt": "safe-combo-0p10-r0p06", "drop": 0.10, "source": "com", "signs": signs_a, "max_blend": 0.55, "adapt_gain": 0.14, "descend_s": 3.2, "pattern": "safe_combo", "scale": 0.06, "filter": "none"},
        {"attempt": "safe-combo-0p10-r0p09", "drop": 0.10, "source": "com", "signs": signs_a, "max_blend": 0.55, "adapt_gain": 0.14, "descend_s": 3.2, "pattern": "safe_combo", "scale": 0.09, "filter": "none"},
        {"attempt": "soft-combo-0p10-r0p09", "drop": 0.10, "source": "com", "signs": signs_a, "max_blend": 0.55, "adapt_gain": 0.14, "descend_s": 3.2, "pattern": "safe_combo", "scale": 0.09, "filter": "soft"},
        {"attempt": "zmp-hold-combo-0p12-r0p10", "drop": 0.12, "source": "com", "signs": signs_a, "max_blend": 0.60, "adapt_gain": 0.13, "descend_s": 3.6, "return_s": 1.8, "pattern": "safe_combo", "scale": 0.10, "filter": "zmp_hold"},
    ]
    result = {
        "planning_gate": {
            "team_validation_mode": "manual-pass",
            "spec_delta": "M19 probes the safe-combo residual basis as a depth curriculum before launching a longer residual fine-tune.",
            "perspectives": {
                "product": "tests the next visible squat blocker identified in ROADMAP",
                "architecture": "reuses exp59 safe_combo basis and sweeps target drop, teacher blend, residual scale, and safety filters",
                "security": "no credentials or external side effects",
                "qa": "native sweep logs CoM/ZMP margins, stance, pose, contact, return, fall",
                "skeptic": "scale sweeps may still hit the same support/ZMP wall before 8cm",
            },
            "dod": [
                "native JSON per CoM/ZMP feedback variant",
                "summary identifies the stable curriculum boundary toward the 8cm M19 gate",
            ],
        },
        "runs": [],
    }
