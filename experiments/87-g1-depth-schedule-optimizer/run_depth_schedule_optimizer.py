"""Search depth schedules around the recoverable three-phase G1 squat teacher."""
...
def variants() -> list[dict[str, Any]]:
    ...
    return [
        {**teacher, "attempt": "teacher-safe-5cm", "max_blend": 0.49, "residual_scale": 0.056, "w_height": 100.0, "depth_cap": 0.20},
        {**teacher, "attempt": "sched-5p5-tight-return", "max_blend": 0.492, "residual_scale": 0.056, "w_height": 104.0, "depth_cap": 0.18, "w_stand": 960.0, "return_s": 2.8, "recapture_s": 1.7},
        {**teacher, "attempt": "sched-6cm-support", "max_blend": 0.495, "residual_scale": 0.057, "w_height": 108.0, "depth_cap": 0.18, "w_support": 11200.0, "w_slip": 9800.0, "recapture_drop": 0.105},
        {**teacher, "attempt": "sched-6p5-soft-pose", "max_blend": 0.498, "residual_scale": 0.058, "w_height": 112.0, "depth_cap": 0.17, "knee_amp": 0.078, "hip_bias": 0.135, "w_hip": 300.0},
        {**teacher, "attempt": "sched-7cm-balanced", "max_blend": 0.500, "residual_scale": 0.058, "w_height": 115.0, "depth_cap": 0.16, "recapture_drop": 0.11, "w_depth_cap": 26000.0},
        {**teacher, "attempt": "sched-7cm-long-recap", "max_blend": 0.500, "residual_scale": 0.058, "w_height": 115.0, "depth_cap": 0.17, "recapture_s": 1.9, "return_s": 2.9, "horizon_steps": 8, "foot_kp_xy": 540.0, "foot_force_clip": 420.0},
        {**teacher, "attempt": "sched-8cm-tight-cap", "max_blend": 0.505, "residual_scale": 0.060, "w_height": 120.0, "depth_cap": 0.15, "knee_amp": 0.082, "hip_bias": 0.145, "w_hip": 330.0, "w_depth_cap": 30000.0},
        {**teacher, "attempt": "sched-8cm-pose", "max_blend": 0.505, "residual_scale": 0.060, "w_height": 122.0, "depth_cap": 0.18, "knee_amp": 0.085, "hip_bias": 0.15, "w_knee": 62.0, "w_hip": 340.0, "recapture_drop": 0.11},
        {**teacher, "attempt": "sched-8cm-return-heavy", "max_blend": 0.505, "residual_scale": 0.060, "w_height": 120.0, "depth_cap": 0.18, "w_stand": 1100.0, "return_joint_kp": 50.0, "return_torque_clip": 70.0, "return_s": 2.7},
        {**teacher, "attempt": "sched-8cm-low-residual", "max_blend": 0.505, "residual_scale": 0.054, "w_height": 122.0, "depth_cap": 0.18, "knee_amp": 0.082, "hip_bias": 0.145},
    ]
...
def write_summary(result: dict[str, Any], out_dir: Path) -> None:
    ...
    (out_dir / "depth-schedule-optimizer-summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    out_dir = VERIFY / "depth-schedule-optimizer"
    ...
    result = {
        ...
        "planning_gate": {
            ...
            "spec_delta": "M19 now evaluates depth schedules around exp86's recoverable 3-phase controller before moving to learned finetuning.",
            "perspectives": {
                "product": "tests whether the same G1 can reach the visible squat gate by schedule search before adding another training loop",
                "architecture": "keeps exp86's 3-phase controller and adds full-rollout optimizer scoring over schedule variants",
                "security": "no credentials or external side effects",
                "qa": "native sweep records raw JSON, visible gate metrics, and optimizer score per schedule",
                "skeptic": "a small schedule grid may only reconfirm the local 5cm recoverable optimum",
            },
            "dod": [
                "raw native JSON per schedule variant",
