"""Probe qfrc stance preload around the recoverable three-phase G1 squat teacher."""
...
def variants() -> list[dict[str, Any]]:
    ...
    return [
        ...
        {**teacher, "attempt": "preload20-return-heavy", "max_blend": 0.505, "residual_scale": 0.060, "w_height": 120.0, "depth_cap": 0.18, "w_stand": 1120.0, "return_joint_kp": 50.0, "return_torque_clip": 70.0, "return_s": 2.7, "preload_force": 20.0, "preload_height_kp": 450.0, "preload_force_clip": 80.0},
        {**teacher, "attempt": "preload35-contact", "max_blend": 0.505, "residual_scale": 0.060, "w_height": 118.0, "depth_cap": 0.18, "w_contact": 760.0, "w_slip": 11200.0, "w_stand": 1120.0, "return_s": 2.8, "preload_force": 35.0, "preload_height_kp": 600.0, "preload_force_clip": 110.0},
        {**teacher, "attempt": "preload50-soft-depth", "max_blend": 0.500, "residual_scale": 0.058, "w_height": 115.0, "depth_cap": 0.16, "recapture_drop": 0.105, "w_depth_cap": 26000.0, "preload_force": 50.0, "preload_height_kp": 650.0, "preload_force_clip": 130.0},
        {**teacher, "attempt": "preload35-knee09", "max_blend": 0.505, "residual_scale": 0.060, "w_height": 120.0, "depth_cap": 0.18, "knee_amp": 0.09, "hip_bias": 0.145, "w_knee": 90.0, "w_hip": 340.0, "preload_force": 35.0, "preload_height_kp": 600.0, "preload_force_clip": 110.0},
        {**teacher, "attempt": "preload20-xy-strong", "max_blend": 0.505, "residual_scale": 0.060, "w_height": 120.0, "depth_cap": 0.18, "foot_kp_xy": 560.0, "foot_force_clip": 430.0, "w_slip": 11800.0, "preload_force": 20.0, "preload_height_kp": 500.0, "preload_force_clip": 90.0},
    ]
...
def write_summary(result: dict[str, Any], out_dir: Path) -> None:
    ...
    (out_dir / "qfrc-stance-preload-wrapper-summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    out_dir = VERIFY / "qfrc-stance-preload-wrapper"
    ...
    result = {
        ...
        "planning_gate": {
            "team_validation_mode": "manual-pass",
                "spec_delta": "M19 moves stance/contact from reward-only tuning into a qfrc control-layer foot-down preload wrapper.",
            "perspectives": {
                "product": "tests whether the same G1 can keep foot contact while attempting the visible squat gate",
                "architecture": "keeps exp87's 3-phase teacher and adds an explicit stance foot preload qfrc term before stepping MuJoCo",
                "security": "no credentials or external side effects",
                "qa": "native sweep records raw JSON, visible gate metrics, preload candidates, and optimizer score per variant",
                "skeptic": "downward preload can improve contact readings while increasing nonphysical joint stress or slip",
            },
            "dod": [
                "raw native JSON per preload variant",
