...
- [x] exp36-43에서 IK/support guard/support-aware reward/reference action-base/soft WBC/contact force와 public squat feasibility를 probe했다. 공개 근거+local static target은 G1 visible/deep squat 가능성을 지지하지만 dynamic policy는 아직 미증명이다.
- [ ] **재개** — 다음 기술 작업은 QP-lite WBC로 pelvis height, foot pose, CoM/support, vertical momentum, foot force balance, inverse torque penalty를 동시에 푸는 것이다.
