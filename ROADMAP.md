...
- **flagship (채택: A+C)** — experiment 01을 *로컬 VLA eval/서빙 오픈소스 도구*(consumer GPU·Windows/WSL2,
  세그폴트·의존성 해법 내장)로 productize **+** 2번째 정책(π0 또는 ACT)을 같은 하네스에 붙여 동작표현
  비교(#2, ADR 0001 실측). → 도구 + 분석 + 블로그 글감을 한 번에.
- [ ] 도구화: 클린 README + 1-command 재현(setup → eval) + CLI 정리
- [ ] 2번째 정책 통합 + 비교 결과표 (ADR 0001 실측 갱신)
