...
- [x] **후속 다듬기 (2026-06-14)** — go1 메시 깨짐 수정 + **자동 시각 QA 하네스**(qaStep+playwright, Claude가 라이브 자가검증) + command 조이스틱 슬라이더 + 배포 무캡(SHA 업로드) + **Shadow Hand 갤러리 추가**(축 A 폭). 라이브 4종(팔·사족+정책·휴머노이드·손) QA PASS.
- [x] **갤러리 폭 완성 + G1 휴머노이드 정책 (2026-06-14 후속2)** — **축 A**: Spot·G1 floating-base settle + Franka Panda 추가(점박이 근본원인=fast_simplification이 non-watertight Franka shell 붕괴 → 바이너리 STL 무감축으로 해소). 갤러리 **8종**. **축 B**: **G1 휴머노이드 보행 정책 직접 학습**([exp 05](experiments/05-g1-rl-walk/README.md)) — Go1 파이프라인 재사용, 103-d obs + gait phase clock, 번들 씬 byte-parity 0.0, 라이브 closed-loop. 학습 정책 임베디먼트 2종(4족+휴머노이드).
