...
- [x] exp36-119에서 controller/selector/WBC/stance curriculum/corridor PPO/reference/native/ONNX adapter/public XML/mjlab/action-wrapper/future-reference/friction planner/contact-force audit/QP-lite/static ID-QP/dynamic tracking smoke/contact-aware retrain/WBC-MPC primitive selector/terminal stand assist/capability/parity/local-retrain/full-order formulation/longer tracker/deployable WBC stack gate를 probe했다. exp109는 visible pose가 static ID-QP에서는 plausible함을 보였고, exp115는 G1 Moves exact scene parity가 공개 artifact만으로 막혔다고 판정했으며, exp117은 qfrc_applied smoke가 1.40~1.46초 fall로 붕괴했고, exp118은 20k-step local tracker도 drop 2.62cm로 gate에 멀었으며, exp119는 GR00T/SONIC 29-DOF contract와 exp33 browser transport는 맞지만 현재 Windows/Docker-less host에서는 공식 sim2sim 실행이 막힌다고 분리했다.
- [ ] **재개** — 다음 기술 작업은 Linux/WSL2/Ubuntu 환경에서 GR00T/SONIC sim2sim을 Git LFS/model/Docker/GPU 조건으로 실행해 29-DOF G1 trace를 exp33 DDS/browser gate로 흘리고, exp29 visible gate와 browser replay를 닫는 것이다. 대안은 동등한 deployable WBC trace를 확보해 같은 gate에 넣는 것이다.

완료 기준: 🟨 M19는 균형 prior와 micro-dip/controller evidence는 확보했고 8cm depth/contact/slip+knee까지 동시에 보였으며 static ID-QP상 visible target 자체는 plausible하다고 분리했지만, stepwise dynamic 추종/contact-aware retrain/primitive MPC/terminal assist/upstream parity/local short retrain/full-order qfrc formulation smoke/20k-step local tracker gate는 실패했다. exp119로 GR00T/SONIC 통합 후보는 29-DOF/browser contract가 맞는다고 확인했지만 "보이는 스쿼트"는 아직 완료가 아니다. 완료 조건은 exp29 visible gate를 native rollout과 browser replay가 동시에 통과하는 것이다. 증거: [exp15](experiments/15-g1-skill-baseline/README.md), [exp18](experiments/18-g1-squat-reward-smoke/README.md), [exp19](experiments/19-g1-squat-recovery-longrun/README.md), [exp20](experiments/20-g1-squat-reference-tracking/README.md), [exp21](experiments/21-g1-stabilizer-init-probe/README.md), [exp22](experiments/22-g1-squat-depth-finetune/README.md), [exp23](experiments/23-g1-squat-target-sanity/README.md), [exp24](experiments/24-g1-squat-skill-design/README.md), [exp25](experiments/25-g1-squat-depth-curriculum/README.md), [exp28](experiments/28-g1-controlled-squat-stage0p74/README.md), [exp29](experiments/29-g1-visible-squat-feasibility/README.md), [exp30](experiments/30-g1-visible-squat-controller/README.md), [exp34](experiments/34-g1-guarded-descent-controller/README.md), [exp35](experiments/35-g1-stance-anchored-squat-controller/README.md), [exp36](experiments/36-g1-wbc-ik-squat-prototype/README.md)-[exp119](experiments/119-g1-deployable-wbc-stack-integration-gate/README.md).
