...
> 컨슈머 GPU에서 VLA(Vision-Language-Action) 정책을 직접 돌리고, 서로 다른 **동작 표현(action representation)** 두 가지를 같은 벤치마크로 실측 비교한 뒤, 브라우저에서 학습 정책 3종을 live closed-loop로 구동·조작·검증하는 피지컬 AI 학습 레포.
...
3. **[browser robot policy platform](experiments/03-digital-twin/web/README.md)** — 하드웨어 없이 브라우저에서 도는 **인터랙티브 3D 임베디먼트 갤러리**. 그중 셋은 **내가 직접 RL로 학습한 보행 정책이 onnxruntime-web으로 실시간 closed-loop** 구동(4족 Go1 · 휴머노이드 G1 · 4족 Spot). 키보드로 보행 command를 바꾸고, 마우스로 팔 EE를 텔레옵하고, command/rough terrain QA로 “걷는다”를 검증한다. → **라이브: [physical-ai-arm.askewly.com](https://physical-ai-arm.askewly.com)**
...
**“걷는다” 다음의 질문까지 측정** — M12에서 `command_sweep.mjs`를 추가해 Go1·Spot의 forward, strafe, turn, diagonal을 로컬+라이브에서 자동 측정했다. 낮은 rough curb scene에서도 낙상·NaN·콘솔에러 0으로 통과했고, yaw convention과 Spot rough drift 같은 한계는 표로 남겼다. → [exp 07](experiments/07-command-terrain-robustness/README.md)
