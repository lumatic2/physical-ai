...
> 컨슈머 GPU에서 VLA(Vision-Language-Action) 정책을 직접 돌리고, 서로 다른 **동작 표현(action representation)** 두 가지를 같은 벤치마크로 실측 비교한 뒤, 브라우저에서 학습 정책 3종을 live closed-loop로 구동한 피지컬 AI 학습·리서치 레포.
...
3. **[digital-twin](experiments/03-digital-twin/web/README.md)** — 하드웨어 없이 브라우저에서 도는 **인터랙티브 3D 임베디먼트 갤러리**. 그중 셋은 **내가 직접 RL로 학습한 보행 정책이 onnxruntime-web으로 실시간 closed-loop** 구동(4족 Go1 · 휴머노이드 G1 · 4족 Spot). 키보드로 보행 로봇을 조종하고, 마우스로 팔 EE를 텔레옵한다. → **라이브: [physical-ai-arm.askewly.com](https://physical-ai-arm.askewly.com)**
