...
**정식 비교는 동일 task 집합(matched)** — experiment 01 OpenVLA는 libero_spatial 첫 3 task만 측정했으므로, 그 3 task로 π0.5를 매칭:

| 비교 | 이산 토큰(OpenVLA) | flow-matching(π0.5) | 검정 |
|------|-------------------|---------------------|------|
| **matched 3 task** (apples-to-apples) | 11/15 = 73.3% | 148/150 = 98.7% | Fisher exact p=6.1e-4 |
| (참고) full-suite *통제 안 됨* | 15(3 task) → 73.3% | 500(10 task) → 97.6% | task 수 달라 비교 무효 |

- **결과**: 동일 task에서 flow-matching이 이산 토큰을 **+25pp** 앞섬(p<0.001). ⚠ 단 하네스·전처리 차이가 섞여 *순수 동작표현 효과*로 단정 불가 — 방향·크기의 강한 증거이되 통제된 인과는 아님(ADR 0003 측정조건 병기).
- **타당성**: π0.5 수치가 openpi 논문 LIBERO 대(~97-98%)와 *일관* — 셋업이 크게 어긋나지 않았다는 시사(증명 아님). HF 포트 provenance·norm_stats 정합성은 미검증(experiment 02 caveat).
- ⚠ full-suite head-to-head 는 OpenVLA 10 task 재측정 시 가능(보류, ADR 0003). 직접 회귀(ACT) 3번째 점은 M6(ADR 0002).
