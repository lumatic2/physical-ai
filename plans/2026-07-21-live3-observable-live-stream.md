...
- Milestone: running session의 dual-camera/state/action/event를 source와 timestamp가 보이는 live stream으로 전달한다.
...
- 관측: drop/reorder count, sync delta, queue depth와 subscriber lag를 노출한다.
...
- [ ] **step-3 — telemetry-event-alignment**
  - Artifact: state, proposed/executed action, policy/event/result의 frame alignment.
  ...
  - Failure probe: action without request/source, future event parent, timestamp skew가 FAIL한다.
...
## Verification / DoD
- 실제 session의 dual-camera와 state/action/event가 read-only browser에서 한 시간축으로 관찰된다.
