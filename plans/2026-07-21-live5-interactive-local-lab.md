...
- Milestone: one-command local lab과 검증된 recorded public proof를 완성한다.
...
- frontend: 기존 arm-lab components에 supported instruction/policy/session controls를 추가한다.
- design: live/recorded mode를 강하게 분리하고 control 권한·stop state를 상시 표시한다.
...
- [ ] **step-2 — instruction-policy-controls**
  - Artifact: supported task/instruction, policy, initial state와 start/pause/resume/stop controls.
  ...
  - Commit: changeset 2 — safe experiment controls.
- [ ] **step-3 — live-laboratory-panels**
  - Artifact: dual-camera, current source event, action/state, latency, health와 stop reason panels.
  - Files: live panels/styles/QA, changeset.
  - Dependencies: step-2
  - Verify: live summary sync/drop/source/health and desktop/mobile layout PASS.
  - Failure probe: chain-of-thought label, stale frame, recorded/live relabel이 FAIL한다.
...
## Verification / DoD
- 사용자가 local에서 지시·정책을 선택해 실제 실행·중단·관찰·기록하고 public에서는 검증된 recording만 본다.
