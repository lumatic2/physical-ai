...
- Milestone: supported instruction과 policy를 선택해 pause/stop/timeout/action limit가 보장되는 session을 실행한다.
...
## 스캐폴딩 결정
- source-of-truth: GEN task catalog와 `physical-ai-experiment-session-v1` state machine이다.
...
- 관측: instruction source, task id, policy, transition, stop reason과 executed action count를 기록한다.
...
- [ ] **step-5 — ten-session-safety-gate**
  - Artifact: 두 policy와 여러 task의 10 operator-controlled session 통합 report.
  - Files: run manifest, verify report, changeset, final report.
  - Dependencies: step-1, step-2, step-3, step-4
  - Verify: 10/10 terminal/cleanup, supported instruction, action/stop provenance PASS.
  - Failure probe: missing session, wrong task binding, unrecorded stop이 FAIL한다.
  - Commit: changeset 5 — safe experiment session gate.
